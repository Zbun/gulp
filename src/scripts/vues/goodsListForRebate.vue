      ...
      <div class="content">
        ...
        <div class="body">
          <div class="content" style="width:1000px">
            ...
            <input type="text" placeholder="输入名称关键字" class="com-ipt large" maxlength="50" v-model="dataOut.search.ProName" @keydown.enter="initPageData(1)">
                  ...
                  <table class="text-center table table-hover table-odd">
                    ...
                    <tbody>
                      <tr v-for="item of dataIn.dataTable">
                        <td>
                          <label class="checkbox">
                            <input type="checkbox" :ProId="'p'+item.uid" :value="item.uid" @change="proId_Change($event,item)" v-model="arrChkProId"><span class="icon"></span></label>
                        </td>
                        <td class="">
                          <label class="block" :for="'p'+item.uid">{{item.Code}}</label>
                        </td>
                        <td class="text-nowrap">
                          <label class="block" :for="'p'+item.uid">{{item.ProId}}</label>
                        </td>
                        <td width="200px">
                          <label class="text-left text-nowrap block" :for="'p'+item.uid">
                            {{item.ProName}}
                          </label>
                        </td>
                        <td class="text-nowrap">
                          <label class="block" :for="'p'+item.uid">{{item.Color}}</label>
                        </td>
                        <td class="text-nowrap">
                          <label class="block" :for="'p'+item.uid">{{item.ProPrice|currency}}</label>
                        </td>
                        <td class="text-nowrap">
                          <label class="block" :for="'p'+item.uid">{{item.ProCount}}</label>
                        </td>
                        <td>
                          <label class="text-nowrap block text-left" :for="'p'+item.uid">{{item.Brand}}</label>
                        </td>

                        <td class="text-nowrap">
                          <label class="block" :for="'p'+item.uid">{{item.Category}}</label>
                        </td>
                        <td class="text-nowrap">
                          <label class="block" :for="'p'+item.uid">{{item.Series}}</label>
                        </td>
                        <td class="text-nowrap">
                          <label class="block" :for="'p'+item.uid">{{item.Standard}}</label>
                        </td>
                      </tr>
    ...
    export default {
        ...
        computed: {
            computedIsAllChecked() {
                ...
                let is = !!l;
                for (let i = 0; i < l; i++) {
                    if (_.indexOf(this.arrChkProId, this.dataIn.dataTable[i].uid) == -1) {
                        is = false;
                        break;
                    }
                }
        ...
        methods: {
            initPageData(pageIndex = 1) {
                ...
                para.SupplierId = this.supplierId;
                ...
                var me = this;
                fetchData({
                    API: '/api/Product/ListForRebate',
                    para,
                    callback(data) {
                        // console.log(data);
                        me.dataIn.inited = true;
                        data.Data.forEach(item => {
                            item.uid = item.ProId + item.Code;
                            if (_.indexOf(this.arrChkProId, item.uid) > -1) {
                                this.arr
                            }
                        })
            ...
            all_Change(e) {
                ...
                this.dataIn.dataTable.forEach(item => {
                    let i = _.indexOf(me.arrChkProId, item.uid);
                    if (i > -1) {
                        if (!isChked) {
                            this.arrChkProId.splice(i, 1);
                        }
                    } else if (isChked) {
                        this.arrChkProId.push(item.uid);
            ...
            selectThisPro(item) {
                ...
                obj.ProCount = item.ProCount; //库存
                obj.ProCount1 = item.ProCount; //库存
                ...
                if (this.action == 1) {
                    obj.PRCode = item.Code
                };
                if (this.action == 2) {
                    obj.SICode = item.Code
                };
                if (this.isLonely) {
                    this.goodsAddedList = [];
                }
            ...
            onSubmit_Click() {
                ...
                this.arrChkProId.forEach(item => {
                    arrTemp.push(_.find(this.dataIn.dataTable, {
                        uid: item
                    }));
                })
                this.goodsAddedList = arrTemp.filter(item => {
                    return item !== undefined
                });
                this.$dispatch('on-add-after', this.goodsAddedList);
                this.show = false;
            },
