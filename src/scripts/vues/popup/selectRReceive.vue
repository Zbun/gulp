<template>
  <div style="display:inline-block">
    <span class="select-box" @click="selectOrder()">
      <input type="text" :title="editInfo.RReceivingCode" placeholder="选择采购退货出库单" readonly v-model="editInfo.RReceivingCode">
      <i class="icon" title="点击选择采购退货出库单编号"></i>
    </span>
    <div class="popup" v-if="show">
      <div class="content">
        <h2 class="title">
          <a href="javascript:;" class="close" title="关闭" @click="onCancel_Click">&times;</a>选择采购退货出库单</h2>
        <div class="body">
            ...
            <div class="bg-muted border-box mt-1 p10 pb20 pl20 pr20 bg-body" v-show="isItemClick">
              <div class="ml10 mb10" style="max-width:80%">
                <div class="clearfix align-btn">
                  <b>
                    <span class="font-small">采购退货出库单号：{{objCurItem.Code}}</span>
                  </b>
                </div>
...
export default {
  ...
  data() {
    return {
      ...
      objCurItem: {},
      APILIST: {
        LIST: 'api/RReceiving/List',
        PREVIEW: '/API/RReceiving/Preview', //预览
      },
      dataIn: {
        inited: false,
        dataTable: {}
  ...
  methods: {
    ...
    onSubmit_Click() {
      ...
      objTemp.RReceivingCode = objTemp.Code;
      delete objTemp.Code;
