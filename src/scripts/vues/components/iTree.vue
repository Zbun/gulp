<template>
  <li :class="['parent',{act: isChecked}]">
    ...
    <ul v-show="open" v-if="isFolder" class="child">
      <i-tree v-for="(item, i) in model.Child" @upName="upName"  @cancelAct="cancelAct" :choose="choose" :indexArr="temIndexArr" :indexNum="i" :key="i" :model="item"></i-tree>
    </ul>
 </li>
</template>
...
export default {
  ...
  methods: {
    checkThis () {
      ...
      if (this.temIndexArr.length >= 5){
        return false;
      }
      this.$emit('cancelAct', this.temIndexArr);
      this.$emit('upName', this.model.Name, this.model.Id);
    },
    upName(...args){
  ...
  created(){
    //先判断当前选项是否是默认选项
    if (this.arrayEqual(this.temIndexArr, this.choose)){
      this.isChecked = true;
    }else{
      this.isChecked = false;
    }
