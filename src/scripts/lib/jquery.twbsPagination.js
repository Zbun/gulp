...
var pagination = function ($, window, document, undefined) {
  ...
  TwbsPagination.prototype = {
    ...
    show: function (page, go) {
      ...
      this.render(this.getPages(page), go);
      this.setupEvents();
    ...
    buildListItems: function (pages, go) {
      ...
      this.$go.find('input').val(this.$element.data('pagego') || 1);
      return listItems;
    },
    ...
    buildItem: function (type, page) {
      ...
      if (type !== 'split') {
        $itemContainer.append($itemContent.attr('href', this.makeHref(page)).text(itemText));
        if (type == 'next') {
      ...
      if (this.options.goVal) {
        ...
        this.$go.find('.btn').click(function () {
          ...
          if (pageGo > 0 && pageGo <= _this.options.totalPages) {
            ...
            _this.$element.data('pagego', pageGo);
