<template>
  <div>
    ...
    <table class="table text-center table-hover table-odd table-bordered mt-1 chks">
      ...
      <tbody>
        <tr v-for="item of dataIn.dataTable">
          ...
          <td>
            <div class="action">
              <a href="javascript:;" v-if="item.ExpressStatus==0" @click="express_Click(item)" class="item">发货</a>
            </div>
          </td>
        </tr>
      </tbody>
    ...
    <popup-express :show.sync="isShowExpress" :code="curCode" :express-list="dataIn.expressList" @on-submit-click="dispatchExpress()"></popup-express>
  </div>
</template>
<script>
import popupExpress from './../popup/popupExpress.vue';
export default {
  props: {
    customerList: [],
    isRefreshIn: false,
