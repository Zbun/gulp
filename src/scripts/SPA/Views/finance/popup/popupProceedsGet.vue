<template>
  <div>
    <div class="popup" v-if="show">
      <div class="content">
        <h2 class="title"><a href="javascript:;" class="close" title="关闭" @click="onCancel_Click">&times;</a>收款</h2>
        <div class="body">
                ...
                <table class="table-intro lh36">
                  <tr>
                    <td style="width:60%">
                      ...
                      <span class="content">{{dataIn.datalist.CustomerName}}</span>
                    </td>
    ...
    initPageData() {
      ...
      fetchData({
        ...
        callback(data) {
          ...
          me.dataOut.PayAmount = data.Amount - data.AmountPayed;
        }
      })
    },
    onOk_Click(event) {
      ...
      if (validatorManu.isEmpty(realPay)) {
        showTips('请填写要收的金额数', 'error');
        return;
      }
      if (this.computedShouldPay > 0) {
        if (realPay < 0) {
          showTips('当应收为正时，实收也需要为正，请检查', 'error');
          return;
        }
        if (realPay > this.computedShouldPay) {
          showTips('实收金额应不大于应收金额，请检查', 'error');
          return;
        }
      } else {
        if (realPay > 0) {
          showTips('当应收为负时，实收也需要为负，请检查', 'error');
          return;
        }
        if (realPay < this.computedShouldPay) {
          showTips('实收为负时，实收金额应不小于应收金额，请检查', 'error');
          return;
        }
      }
      let para = {};
      para.id = this.dataIn.datalist.CustomerId;
      let objTemp = {};
