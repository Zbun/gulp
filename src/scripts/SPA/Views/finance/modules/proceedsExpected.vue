<template>
  <div>
    ...
    <table class="table text-center table-hover table-odd table-bordered datalist">
      ...
      <tbody>
        <tr v-for="item of dataIn.dataTable">
          ...
          <td>
            <p class="text-nowrap"> {{item.Status|proceedsStatus}}</p>
          </td>
          <td>
            <div class="action">
              <a href="javascript:;" v-if="item.Status!=2" @click="get_Click(item.BillCode,item.CustomerId)" class="item">收款</a>
    ...
    <popupproceeds-get :show.sync="isShowPayOrder" :customer-id="curCustomerId" @on-after-submit="payAfter" :bill-code="curBillCode"></popupproceeds-get>
  </div>
</template>
<script>
import popupproceedsGet from './../popup/popupproceedsGet.vue';
...
export default {
  ...
  computed: {
    computedProceedsStatus() {
      return dictionary['proceedsStatus'];
    },
