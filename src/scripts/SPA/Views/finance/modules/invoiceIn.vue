<template>
  <div>
    ...
    <table class="table text-center table-bordered datalist">
      <thead>
        <tr>
          <th class="td-warning-pure" rowspan="2">采购单号</th>
          <th class="td-warning-pure" rowspan="2">采购金额（元）</th>
          <th class="td-warning-pure" rowspan="2">供应商</th>
      ...
      <tbody>
        <template v-for="item of dataIn.dataTable">
          <tr>
            ...
            <td :rowspan="item.Receipts.length+1">
              <p class="text-nowrap">{{item.TotalPrice}}</p>
            </td>
            <td :rowspan="item.Receipts.length+1">
              {{item.SupplierName}}
            </td>
    ...
    <operate-invoice :show.sync="isShowOperateInvoice" :obj-cur-order="objCurOrder" :is-supplier="isSupplier" @on-after-submit="dispatchSubmit()"></operate-invoice>
  </div>
</template>
<script>
...
export default {
  ...
  data() {
    return {
      ...
      isSupplier: 1,
      APILIST: {
        LIST: '/Api/Finance/SupplierReceiptList',
      },
      dataIn: {
        inited: false,
        dataTable: [],
  ...
  methods: {
    ...
    delReceipt(rp) {
      ...
      dialog({
        ...
        ok() {
          fetchData({
            API: '/Api/Finance/SupplierReceiptHistoryDelete',
            para: {
              Id: rp.Id
            },
            callback() {
