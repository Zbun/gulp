<template>
  <div>
    ...
    <div class="clearfix border-box bg-muted p20 mt20">
      <div class="fr">
        <button class="btn btn-warning" v-if="rightsApi.Insert" :class="{'btn-disabled':arrChkCode.length!=1}" @click="scanSn_Click()">入库</button>
    ...
    <table class="table text-center table-hover table-odd table-bordered  mt-1">
      ...
      <tbody>
        <tr v-for="item of dataIn.dataTable">
          ...
          <td>
            <a v-link="{ name: 'purchaseOrderCheck', params:{ pocode: item.Code}}" class="highlight">{{item.Code}}</a>
          </td>
          <td>
            {{item.SupplierName}}
          </td>
    ...
    <scan-sn :show.sync="isShowScanSn" pop-title="入库串号扫描" :code="curCode" @on-after-submit="dispatchScanSn()" :store-id="curStoreId" is-by-standard="1" :api-list="rightsApi"></scan-sn>
  </div>
</template>
<script>
...
export default {
  ...
  data() {
    return {
      ...
      arrChkCode: [],
      APILIST: {
        LIST: 'api/PurchasingContract/List'
      },
      dataIn: {
        inited: false,
        dataTable: {}
