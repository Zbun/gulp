<template>
  <div>
    <div class="search clearfix mt20">
      ...
      <input type="search" class="com-ipt width" placeholder="输入供应商" v-model="dataOut.search.SupplierName" @keydown.enter="initPageData(1)" maxlength="50">
      </span>
      ...
      <tbody>
        <tr v-for="item of dataIn.dataTable">
          ...
          <td class="text-nowrap">
            <a v-link="{ name: 'PRLibraryOrderCheck', params:{lproCode:item.Code}}" class="highlight">{{item.Code}}</a>
          </td>
          <td>
            {{item.SupplierName}}
          </td>
