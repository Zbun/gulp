<template>
  <div>
    ...
    <div class="clearfix border-box bg-muted p20 mt20">
      <div class="fr">
        <button class="btn btn-warning" v-if="rightsApi.In" :class="{'btn-disabled':arrChkCode.length!=1}" @click="libraryIn_Click()">入库</button>
    ...
    <library-in :show.sync="isShowLibraryIn" :api-insert="rightsApi.In" @on-added-ok="dispatchAddOk()" :cur-Item="curItem"></library-in>
  </div>
</template>
<script>
import libraryIn from './../popup/dispatchLibraryIn.vue';
export default {
  props: {
    isRefreshIn: false,
    rightsApi: {}
  ...
  methods: {
    ...
    libraryIn_Click() {
      this.curItem = _.find(this.dataIn.dataTable, {
        Code: this.arrChkCode[0]
      });
      this.isShowLibraryIn = true;
    },
    dispatchAddOk() {
