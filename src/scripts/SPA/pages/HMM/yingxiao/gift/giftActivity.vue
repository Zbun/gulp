    ...
    <div class="content-field  clearfix">
      <div class="search-box clearfix">
        <span class="fr">
          ...
          <router-link  class="btn btn-primary ml5" :to="{name:'HMMgiftActivityEdit'}" >新增活动</router-link>
        </span>
        <span class="tab-bar">
            ...
            <td>
              <div class="action-field">
                <template v-if="item.IsShowBtnEdit==1">
                  <router-link class="item" :to="{name:'HMMgiftActivityEdit',query:{id:item.Id}}">编辑</router-link>
                  <a href="javascript:;" class="item" @click="del(item)">删除</a>
                </template>
                <template v-if="item.IsShowBtnStop==1">
                  <router-link class="item" :to="{name:'HMMgiftActivityEdit',query:{id:item.Id,isCheck:1}}">查看</router-link>
                  <a href="javascript:;" class="item" @click="end(item)">结束</a>
                </template>
                <template v-if="item.IsShowBtnCopy==1">
                  <router-link class="item" :to="{name:'HMMgiftActivityEdit',query:{id:item.Id,isCopy:1}}">复制</router-link>
                  <a href="javascript:;" class="item" @click="del(item)">删除</a>
                </template>
              </div>
            </td>
  ...
  methods: {
    ...
    del(item) { //删除
      dialog({
        ...
        ok: () => {
          fetchData({
            API: '/api/Activity/ActivityGiftDelete',
            para: {
              Id: item.Id,
            },
            callback: data => {
