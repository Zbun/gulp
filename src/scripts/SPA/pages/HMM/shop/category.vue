        ...
        <table class="table table-bordered table-hover datalist">
          ...
          <tbody>
            <tr v-for="(item,index) in curItem.Child">
              ...
              <td>
                <a href="javascript:;" class="icon icon-up" @click="up(item,index)" title="向上" :class="{disabled:index==0}"></a>
                <a href="javascript:;" class="icon icon-down" @click="down(item,index)" :class="{disabled:index+1==curItem.Child.length}" title="向下"></a>
              </td>
              <td>
...
export default {
  ...
  methods: {
    ...
    up(item, index) {
      fetchData({
        API: 'api/ShopCategory/CategoryWeightUP',
        para: {
          Id: item.Id
        },
        callback: data => {
    ...
    down(item, index) {
      fetchData({
        API: 'api/ShopCategory/CategoryWeightdown',
        para: {
          Id: item.Id
        },
        callback: data => {
    ...
    enable(e, item) {
      ...
      fetchData({
        API: '/api/ShopCategory/CategoryEnable',
        para: {
          Id: item.Id,
          Enable: isChked - 0
        },
    ...
    del(item, index) {
      dialog({
        ...
        ok: () => {
          fetchData({
            API: '/api/ShopCategory/CategoryDelete',
            para: {
              Id: item.Id
            },
            callback: data => {
