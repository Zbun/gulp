<template>
  <div class="main">
      ...
      <table class="table table-bordered datalist table-hover">
        ...
        <tbody>
          <tr v-for="item in dataIn.arrDataList">
            <td :title="item.Name">
              <i class="iconfont icon-dianhua" style="color:#00AFDF"></i> {{item.Phone}}
            </td>
    ...
    <div class="popup" v-if="show">
      <div class="content">
        <h2 class="title"><a href="javascript:;" class="close" title="关闭" @click="cancelClick">&times;</a>电话客服{{isEdit==1?'修改':'添加'}}</h2>
        <div class="body">
          <div class="content" style="width:400px">
            <div class="set ">
              <dl class="item">
                <dt class="title required-mark">电话号码</dt>
                <dd class="content">
                  <input type="text" placeholder="请输入电话号码" class="com-ipt large" v-model="objEditItem.Phone" data-validate="required telnum" maxlength="20">
                </dd>
              </dl>
              <dl class="item">
...
export default {
  ...
  methods: {
    initPageData() {
      let para = {};
      fetchData({
        API: '/api/Supplier/ShopCustomerServicePhoneList',
        hideLoading: true,
        para,
        callback: data => {
    ...
    submitClick() {
      ...
      fetchData({
        API: '/api/Supplier/ShopCustomerServicePhoneEdit',
    ...
    del(item) {
      dialog({
        title: '删除',
        content: `确认删除"${item.Phone}"？`,
        ok: () => {
          fetchData({
            API: '/api/Supplier/ShopCustomerServicePhoneDelete',
            para: {
              Id: item.Id
            },
            callback: () => {
